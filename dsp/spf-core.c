	...
	case APM_CMD_RSP_GET_SPF_STATE:
		spf_status_rsp =
				GPR_PKT_GET_PAYLOAD(
					struct apm_cmd_rsp_get_spf_status_t,
					data);
		dev_info(&adev->dev, "%s: success response received", __func__);
...
static int spf_core_platform_driver_probe(struct platform_device *pdev)
{
	...
	spf_core_priv = devm_kzalloc(&pdev->dev,
	sizeof(struct spf_core_private), GFP_KERNEL);
	if (!spf_core_priv)
		return -ENOMEM;
	...
	INIT_WORK(&spf_core_priv->add_chld_dev_work,
