...
static int __init adsp_loader_gecko_init(void)
{
	return platform_driver_register(&adsp_loader_driver);
}
module_init(adsp_loader_gecko_init);

static void __exit adsp_loader_gecko_exit(void)
{
	platform_driver_unregister(&adsp_loader_driver);
}
module_exit(adsp_loader_gecko_exit);

MODULE_DESCRIPTION("ADSP Loader Gecko module");
