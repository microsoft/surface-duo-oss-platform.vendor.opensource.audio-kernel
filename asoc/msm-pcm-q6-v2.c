...
/* default value is read from device tree */
static char const *msm_pcm_fe_topology_text[] = {
	"DEFAULT", "ULL", "ULL_PP", "LL" };
				...
				struct snd_pcm_hw_params *params)
{
	...
	int dir, ret;
	unsigned long fe_id = soc_prtd->dai_link->id;
			...
			const char *name_suffix, int session_type, int channels)
{
	...
	ret = snd_soc_add_platform_controls(rtd->platform, kctl, 1);
	kfree(mixer_name);
	return ret;
}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (strnstr(kcontrol->id.name, "Playback", sizeof(kcontrol->id.name)))
