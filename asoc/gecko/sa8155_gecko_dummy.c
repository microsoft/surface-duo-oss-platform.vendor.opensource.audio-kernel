...
#include "msm-audio-gecko-defs-v2.h"

#define DRV_NAME "sa8155-gecko-asoc-snd"
...
struct snd_soc_card snd_soc_card_auto_msm = {
	.name = "sa8155-adp-star-gecko-snd-card",
};
...
int __init sa8155_gecko_init(void)
{
	pr_debug("%s\n", __func__);
	return platform_driver_register(&sa8155_asoc_machine_driver);
}

void sa8155_gecko_exit(void)
{
	pr_debug("%s\n", __func__);
	platform_driver_unregister(&sa8155_asoc_machine_driver);
}

MODULE_DESCRIPTION("ALSA SoC Machine Driver for Gecko");
